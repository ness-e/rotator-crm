...
export default function Login() {
  ...
  useEffect(() => {
    document.title = `Rotator SU - ${t('login.title')}`;

    // Detección automática de idioma solo en el login si no hay preferencia guardada
    if (!localStorage.getItem('lang')) {
      const browserLang = navigator.language || navigator.userLanguage;
      let detected = 'en';
      if (browserLang.startsWith('es')) detected = 'es';
      else if (browserLang.startsWith('pt')) detected = 'pt';
  ...
  const submit = handleSubmit(async (values) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errorData = await res.json().catch(() => ({ error: t('login.authError') }));
