...
export default function AdminLicenses() {
  ...
  const lSchema = useMemo(() => licenseSchema(t), [t])
  ...
  const form = useForm({
    resolver: zodResolver(lSchema), defaultValues })
...
${t('licenses.form.mobile')}: ${item.limitMobileUsers} | ${t('licenses.form.admins')}: ${item.limitAdmins}
  ...
  const columns = [
    ...
    { 
      key: 'status', label: t('common.status'), render: (v) => <Badge variant={v === 'ACTIVE' ? 'success' : v === 'EXPIRED' ? 'destructive' : 'secondary'}>{t(`common.${v.toLowerCase()}`, { defaultValue: v })}</Badge> 
    },
    { 
      key: 'expirationDate', 
