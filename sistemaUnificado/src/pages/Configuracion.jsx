...
function SettingsTab() {
    ...
    const handleSave = async () => {
        ...
        if (invalidEmails) {
            return toast({ 
                title: t('configuration.validationError', 'Error de Validación'), 
                description: t('configuration.emailSuffixError', 'Todos los correos de reporte deben tener el sufijo @rotatorsurvey.com'), 
                variant: 'destructive' 
            });
        }
        setSaving(true)
        try {
            ...
            if (res.ok) toast({ title: t('configuration.saveSuccess', 'Configuración guardada') })
            else throw new Error('Error saving')
        } catch (e) {
            toast({ title: t('configuration.error', 'Error'), description: t('configuration.saveError', 'No se pudo guardar la configuración'), variant: 'destructive' })
...
function RolesTab() {
    ...
    const handleEdit = (role) => {
        if (role.name === 'MASTER') {
            return toast({ 
                title: t('roles.protected', 'Rol de Sistema Protegido'), 
                description: t('roles.protectedDesc', 'El rol MASTER no puede ser modificado para garantizar la integridad del sistema.'),
                variant: 'destructive' 
            })
        }
        setEditing(role)
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        if (!formData.name) return toast({ title: t('roles.nameRequired', 'Nombre requerido'), variant: 'destructive' })
        try {
            ...
            toast({ title: isNew ? t('roles.toast.created', 'Rol creado') : t('roles.toast.updated', 'Rol actualizado') })
            setOpen(false)
            loadRoles()
        } catch (err) {
            toast({ title: t('configuration.error', 'Error'), description: err.message, variant: 'destructive' })
        }
    }
    const handleDelete = async (role) => {
        ...
        try {
            ...
            toast({ title: t('roles.toast.deleted', 'Rol eliminado con éxito') })
            loadRoles()
        } catch (err) {
            toast({ title: t('roles.errorDelete', 'Error al eliminar'), description: err.message, variant: 'destructive' })
        }
    }
    return (
        <div className="space-y-6">
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                toast({ title: t('backup.toast.downloaded', 'Backup descargado') })
            } else throw new Error('Error')
        } catch (e) {
            toast({ title: t('configuration.error', 'Error'), description: t('backup.toast.downloadError', 'No se pudo descargar el backup'), variant: 'destructive' })
        } finally { setDownloading(false) }
    }
    const handleRestore = async () => {
        if (!file) return
        if (!window.confirm(t('backup.restoreConfirm', 'ADVERTENCIA: ¿Estás seguro de restaurar? Esto sobrescribirá TODOS los datos actuales.'))) return
        setRestoring(true)
        ...
        try {
            ...
            if (res.ok) {
                toast({ title: t('backup.toast.restored', 'Restauración completada') })
                setFile(null)
                setTimeout(() => window.location.reload(), 2000)
            } else throw new Error('Restore failed')
        } catch (e) {
            toast({ title: t('configuration.error', 'Error'), description: t('backup.toast.restoreError', 'Falló la restauración'), variant: 'destructive' })
