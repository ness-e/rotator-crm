    ...
    return (
        ...
        >
            <DataTable
                loading={loading}
                columns={[
                    { key: 'id', label: t('common.id'), render: (v) => <span className="font-mono text-xs text-muted-foreground">{v}</span> },
                    {
                        key: 'createdAt',
                        label: t('audit.table.date'),
                        render: (v) => (
                            <span className="font-mono text-xs text-muted-foreground">
                                {v ? format(new Date(v), 'dd MMM yyyy HH:mm:ss', { locale: dateLocale }) : '—'}
                            </span>
                        )
                    },
                    {
                    ...
                    },
                    {
                        ...
                        render: (v) => (
                            <Badge variant="outline" className={`border-0 ${getActionColor(v)} uppercase text-[10px]`}>
                                {t(`audit.actions.${v}`, { defaultValue: v })}
                            </Badge>
                        )
                    },
                    {
