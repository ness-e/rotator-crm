...
import { planVersionSchema, hostingPlanSchema } from '@/lib/validations'
...
export default function AdminPlans({ defaultTab = null }) {
  ...
  // --- SCHEMAS ---
  const vSchema = useMemo(() => planVersionSchema(t), [t])
  const hSchema = useMemo(() => hostingPlanSchema(t), [t])
  ...
  // --- COLUMNS ---
  const vCols = [
    { key: 'id_version', label: t('common.id'), render: v => <span className="font-mono text-xs">{v}</span> },
  ...
  const hCols = [
    { key: 'id', label: t('common.id'), render: v => <span className="font-mono text-xs">{v}</span> },
                ...
                <Form {...vForm}>
                  <form onSubmit={vForm.handleSubmit(onSubmitV)} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <FormField control={vForm.control} name="version_nombre" render={({ field }) => <FormItem><div className="flex items-center gap-2"><FormLabel>{t('plans.versions.form.name')}</FormLabel><InfoHint content={t('plans.versions.tooltips.name')} /></div><FormControl><Input {...field} placeholder={t('plans.versions.form.namePlaceholder')} /></FormControl><FormMessage /></FormItem>} />
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 border p-4 rounded-lg bg-slate-50 dark:bg-slate-900/50">
                      ...
                      <FormField control={vForm.control} name="cuestionarios_concurrentes" render={({ field }) => <FormItem><div className="flex items-center gap-2"><FormLabel className="text-xs">{t('plans.hosting.form.concurrent')}</FormLabel><InfoHint content={t('plans.versions.tooltips.concurrent')} /></div><FormControl><Input type="number" {...field} className="bg-muted" readOnly /></FormControl><FormMessage /></FormItem>} />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
              ...
              <DialogContent className="max-w-xl">
                ...
                <Form {...hForm}>
                  <form onSubmit={hForm.handleSubmit(onSubmitH)} className="space-y-4">
                    ...
                    <FormField control={hForm.control} name="concurrentQuestionnaires" render={({ field }) => <FormItem><FormLabel>{t('plans.hosting.form.concurrent')}</FormLabel><FormControl><Input type="number"{...field} /></FormControl><FormDescription>{t('plans.hosting.description')}</FormDescription><FormMessage /></FormItem>} />
                    <DialogFooter>
