...
$mastersPath = realpath($webRoot . '/../../rotator_masters/masters.php');
// funciones_comunes.php is at webRoot/web/varias/funciones_comunes.php
$funcionesComunesPath = realpath($webRoot . '/web/varias/funciones_comunes.php');
...
// ═══════════════════════════════════════════════════════════════════════════════
$shortenOk = false;
$htaccessPath = realpath($serverRoot . '/.htaccess');
...
// ═══════════════════════════════════════════════════════════════════════════════
$ajaxFile = $webRoot . '/ajaxCheck.php';
...
// ═══════════════════════════════════════════════════════════════════════════════
$faviconPath = $serverRoot . '/favicon.ico';
...
// ═══════════════════════════════════════════════════════════════════════════════
$malware = ['suspected' => 0, 'infected' => 0, 'modifiedFolders' => 0, 'status' => 'ok'];

// Malware signatures to search for
$virusSignatures = [
    'Sitemap:http:', 'function echo_json($', '===undefined){function',
    "+'Of'](", ')+-parseInt(', "['push'](", "['shift']())",
    "']());}}}(", 'if(ndsw===undefined){', ';if(nds',
    'token=function(){return', '*(-parseInt(',
    'urlencode(base64_encode(json_encode(', 'if (is_callable(',
    'function mode()', '= $_COOKIE;', 'return self::$',
    'Jackpot', 'POWERGAMING', 'Game Online', 'google-site-verification',
    "h=h[d]=h[d]||{q:[]", 'product:price:amount', 'Hey siriusly',
    'product-preview__glider', 'TeePublic.Utility.unveil_image',
    '$_REQUEST[', '$license($', '.send()}}(window);',
    'instanceof Array)){', '"undefined"!=typeof Symbol',
    'eval(function(', 'function packToHex($inputToPack)',
    'function fun3($exploded)', '$bitwiseXor',
    'eval(base64_decode("', "eval(base64_decode('",
    'eval(base64_decode(str_replace(',
    '@$_SERVER["HTTP_HOST"].@$_SERVER["REQUEST_URI"];',
    '=openssl_decrypt($', '){global $_', '(){window[',
    '= new Date();var', '){return null};var',
    "=function(){return'\\", '=true;break;case',
    '(sys_get_temp_dir(),time());if(file_put_contents($',
    '$GLOBALS["', '=["\x', ',$$_:', '_$$:',
    "'send','inde'", "+'tr'](", '=new HttpClient(),',
    ')!==-0', '=screen,', "='+token();",
    ';if(navigator[appVersion_var].indexOf(',
    'var appVersion_var="', '\x63\x65";function',
    'window[unescape("',
    "'openssl_private_decrypt','openssl_decrypt',",
];

// Scan from serverRoot (same scope as estructura.php which scans from '../' of its location)
$malwareScanRoot = $serverRoot ?: $webRoot;
if ($malwareScanRoot) {
    $scanMalware = function ($dir, $depth = 0) use (&$scanMalware, &$malware, $virusSignatures) {
        if ($depth > 10)
        ...
        if (!$items)
            return;
        foreach ($items as $item) {
            if ($item === '.' || $item === '..' || $item === 'monitorServer.php' || $item === 'cawi_monitor.php')
                continue;
            $fullPath = "$dir/$item";
            $ext = strtolower(pathinfo($item, PATHINFO_EXTENSION));
            if (is_file($fullPath) && in_array($ext, ['js', 'php', 'txt', 'html', 'htm', 'htaccess', 'htc_'])) {
                $content = @file_get_contents($fullPath);
                if ($content === false)
                    continue;
                foreach ($virusSignatures as $sig) {
            ...
            if (is_dir($fullPath) && substr($item, 0, 1) !== '.') {
                $scanMalware($fullPath, $depth + 1);
            }
        }
    };
    $scanMalware($malwareScanRoot);
}
...
$htaccessInfo = '';
$publicHtaccess = $serverRoot . '/.htaccess';
...
$results['email_test'] = [
    'status' => $mailOk ? 'ok' : 'error',
    'mailFunctionAvailable' => $mailOk,
    'note' => 'Mail function availability check only. Use /email-test endpoint for sending.',
];
...
foreach ($results as $key => $data) {
    ...
    switch ($key) {
        ...
        case 'malware_analysis':
            $message = ($data['infected'] ?? 0) > 0
                ? "⚠ {$data['infected']} archivos infectados detectados"
